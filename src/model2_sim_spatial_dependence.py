...
def sim_model2(x0, steps, ds, n = 10, correlate = True,
                       **kwargs):
    ...
    for i in range(steps):
        ...
        if not correlate:
            ...
            for j in range(n):
                ...
                x_next_i = step_image(x_prev, ds, **kwargs)
                x_next_i = x_next_i.assign_coords(n = j)
                x_next.append(x_next_i)
        
            x_next = xr.concat(x_next, dim = 'n')

            # correlate different simulations for h
            U = uniform.rvs(size = (m, n))
            Uc = A @ U
            J = Uc.argsort().argsort()  ## indexen not ranking
            X_p = x_next.stack(k = ['i', 'j'])
            H = X_p.h.values.T
            # Hs
            Hs = np.sort(H,axis = 1)
            Hc = np.zeros((m, n))
            ...
            x_next = x_next.sel(n = 0 )
            x_next.h[:] = Hc[:,5].reshape((N,N))
...
# =============================================================================
    ...
    X0['t'] = 0
    X0 = X0.assign_coords( i_ind = range(len(X0.i)), j_ind = range(len(X0.j)))
    X0 = X0.stack(i_p = ['i_ind', 'j_ind'])
      
 
# =============================================================================
#   Sigma correlatie matrix
# =============================================================================
    ...
    coordinates = X0.i_p.values 
    coordinates_array = np.asarray(coordinates.tolist())
    ...
    X = X.where(X.area == 1)
        # X = X.assign_coords(n = i)
        # X_list.append(X)
    
    # X.h.where(X.h >0).plot(col = 't', col_wrap = 5)
    import matplotlib.pyplot as plt
    # X = xr.concat(X_list, dim = 'n')
    t = [0, 3, 6, 30]
    X = X.assign_coords(i_plot = X.i / 1000, j_plot = X.j / 1000)
    X['CTH (km)'] = X.h / 1000 
    g = X['CTH (km)'].where(X.h >0).sel(t = t).plot(x = 'i_plot', y = 'j_plot', col = 't')
    for i, ax in enumerate(g.axes.flat):
        ...
        if (i == 0):
            ...
            ax.set(ylabel = '')
    #     ax.axis('equal')
    plt.suptitle('Spatially correlated simulation of CSS MC simulations')
    plt.savefig('../fig/model1_corr/' + 'simulation0', dpi = 300)    
    plt.show()
    ...
    #     Hc[row] = Hs[row][J[row]]
        ...
        # plt.imshow(Hc[:,round(n/2)].reshape((N,N)))
    
    
    # X['h_sim'] = (['t', 'i', 'j'],np.stack([X.h.sel(n = 0, t= 0 ).values, Hc[:,5].reshape((N,N))]) )
    

    # filename = loc_sim + f'simulation2_{method}_T{T}_N{N}'
    

    # X.h_sim.where(X.h_sim >0).plot(col = 't')
